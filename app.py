...
X = df.drop(["Policy Number", "Fraudulent"], axis=1)
y = df["Fraudulent"]
...
def predict_fraudulent_claim(new_customer_data):
    ...
    fraudulent_status = target_encoder.inverse_transform(predicted_label)
    return fraudulent_status[0]
...
st.header('Insurance Fraud Detection System')
...
# Input fields for mobile number data
mobile_number = st.text_input('Mobile Number')

st.write("Or")

# Input fields for email id data
...
if st.button('Predict Fraudulent Claim'):
    ...
    if inputData:
        ...
        if row:
            fraudulent_claim = predict_fraudulent_claim(getCustomerData(row))
            fraud_status = 'Fraudulent' if fraudulent_claim == 1 else 'Not Fraudulent'
            st.subheader(f'Fraudulent Claim Status: {fraud_status}')
            ...
            st.error("Error: No matching data found.")
    else:
